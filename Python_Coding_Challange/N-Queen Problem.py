...
#########################################################################################  
########## Optimized GFG Solution ##########
#########################################################################################            
class Solution:
        
    def solve(self,col,board,ans,leftrow,upperDiagonal,lowerDiagonal,temp,n):
        if col == n:
            ans.append(temp[:])
            return
        ...
        for row in range(n):
            if ( 
                ...
                self.solve(col+1,board,ans,leftrow,upperDiagonal,lowerDiagonal,temp,n)
                temp.pop()
                board[row] = board[row][:col] + "." +board[row][col+1:]
                leftrow[row] = 0
